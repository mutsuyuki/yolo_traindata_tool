...
if __name__ == '__main__':
    ...
    for i, img in enumerate(trans_img):
        cv2.imwrite("my_train_data/JPEGImages/" + base + "_" + str(i) + ".jpg", img)
        shutil.copy("my_train_data/labels/" + base + ".txt", "my_train_data/labels/" + base + "_" + str(i) + ".txt")

    for i, img in enumerate(flip_img):
        file_index = i + len(trans_img)
        cv2.imwrite("my_train_data/JPEGImages/" + base + "_" + str(file_index) + ".jpg", img)
        teacher_data = []
        with open("./my_train_data/labels/" + base + ".txt") as f:
            for line in f:
                data = line.split(" ")
                if (len(data) < 5):
                    break
        ...
        with open("./my_train_data/labels/" + base + "_" + str(file_index) + ".txt", "w") as f:
            file_index = i + len(trans_img)
